...
local icons = require("empty.util.icons")
...
function M.pretty_path(opts)
  ...
  return function(self)
    ...
    path = norm(path)
    local current_root = norm(root.get({ normalize = true }))
    local cwd = norm(root.cwd())

    local norm_path = path
    if is_win() then
      norm_path = norm_path:lower()
      current_root = current_root and current_root:lower() or current_root
      cwd = cwd and cwd:lower() or cwd
    end

    if opts.relative == "cwd" and cwd and norm_path:find(cwd, 1, true) == 1 then
      path = path:sub(#cwd + 2)
    elseif current_root and norm_path:find(current_root, 1, true) == 1 then
      path = path:sub(#current_root + 2)
    end
...
function M.root_dir(opts)
  ...
  local function get()
    local cwd = root.cwd()
    local current_root = root.get({ normalize = true })
    local name = vim.fs.basename(current_root)

    if current_root == cwd then
      return opts.cwd and name
    elseif current_root:find(cwd, 1, true) == 1 then
      return opts.subdirectory and name
    elseif cwd:find(current_root, 1, true) == 1 then
  ...
  return {
    function()
      local name = get()
      return name and ((opts.icon or "") .. " " .. name) or ""
    end,
    cond = function()

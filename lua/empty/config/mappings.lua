local format = require("empty.util.format")
local root = require("empty.util.root")
...
local function open_url()
	...
	if vim.ui and vim.ui.open then
		...
		vim.notify("vim.ui.open is not available", vim.log.levels.ERROR)
	end
end
...
map("n", "<C-j>", "<cmd>NvimTmuxNavigateUp<cr>", { desc = "Go to Lower Window", remap = true })
map("n", "<C-k>", "<cmd>NvimTmuxNavigateRight<cr>", { desc = "Go to Upper Window", remap = true })
...
map({ "n", "x" }, "gx", open_url, { desc = "Open URL" })
...
end, { desc = "Location List (Picker)" })
map("n", "<leader>xQ", function()
	Snacks.picker.qflist()
end, { desc = "Quickfix List (Picker)" })
...
local function terminal_root()
	-- Prefer the last non-terminal buffer so toggle targets the same terminal.
	local buf = vim.api.nvim_get_current_buf()
	if vim.bo[buf].buftype == "terminal" then
		local alt = vim.fn.bufnr("#")
		if alt > 0 and vim.api.nvim_buf_is_valid(alt) then
			return root.get({ buf = alt })
		end
	end
	return root.get()
end
...
local function toggle_root_terminal()
	Snacks.terminal.toggle(nil, { cwd = terminal_root(), win = { position = "float" } })
end

map({ "n", "t" }, "<c-\\>", toggle_root_terminal, { desc = "Floating Terminal" })
...
-- undo tree
map("n", "<leader>t", vim.cmd.UndotreeToggle, { desc = "Toggle Undotree" })
